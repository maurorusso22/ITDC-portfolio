...
def compress(input, window_length=4):
    ...
    while input:
        triple = find_longest_match(input, window_length, seen_input, window_begin_index)
        if (triple):
            output.append(triple)
        
        offset, length, char = triple

        # update what you have seen so far
        seen_input += input[:(length+1)]
